import { companyModel, CompanyModel, companyRoleModel, CompanyRoleModel, staffModel, StaffModel } from 'schemas/index.ts'
import { createColumnHelper, useReactTable, getCoreRowModel, PaginationState } from '@tanstack/react-table'
import { ModelConfig, ColumnsList } from '@/components/ModelRuntimeConfig'
import { match } from 'ts-pattern'
import { useGetApiCompanies, useGetApiCompaniesIdRelationships, useGetApiCompaniesIdStaffMembers } from 'services/Companies.generated.ts'
import { useMemo, useReducer, useState } from 'react'

export const useCompaniesTableColumnHelper = createColumnHelper<CompanyModel>();
export type UseCompaniesTableArgs = {sortBy?: string | undefined, embed?: Array<'companyTypes' | 'relationships'> | undefined, id?: Array<string> | undefined, address?: string | undefined, branch?: string | undefined, name?: string | undefined, typeId?: string | undefined, negotiatorId?: Array<string> | undefined, officeId?: Array<string> | undefined, contactDetail?: Array<string> | undefined, fromArchive?: boolean | undefined, createdFrom?: string | undefined, createdTo?: string | undefined, modifiedFrom?: string | undefined, modifiedTo?: string | undefined, metadata?: Array<string> | undefined, columns: ColumnsList<CompanyModel>};
...
export const getuseCompaniesIdRelationshipsTableColumn = (property:string, modelConfig: ModelConfig<CompanyRoleModel>) => {
      return match(property).with('_links', () => {
      const { label: header, format, width, minWidth } = modelConfig['_links']
...
export const useCompaniesIdStaffMembersTableColumnHelper = createColumnHelper<StaffModel>();
export type UseCompaniesIdStaffMembersTableArgs = {id: string, columns: ColumnsList<StaffModel>};
export const getuseCompaniesIdStaffMembersTableColumn = (property:string, modelConfig: ModelConfig<StaffModel>) => {
      return match(property).with('name', () => {
      const { label: header, format, width, minWidth } = modelConfig['name']

      return useCompaniesIdStaffMembersTableColumnHelper.accessor(row => row.name, {
      id: 'name',
      header,
      cell: info => format(info.getValue()),
      size: width,
...
.with('active', () => {
      ...
      return useCompaniesIdStaffMembersTableColumnHelper.accessor(row => row.active, {
      id: 'active',
      header,
      cell: info => format(info.getValue()),
      size: width,
...
.with('jobTitle', () => {
      ...
      return useCompaniesIdStaffMembersTableColumnHelper.accessor(row => row.jobTitle, {
      id: 'jobTitle',
      header,
      cell: info => format(info.getValue()),
      size: width,
...
.with('workPhone', () => {
      ...
      return useCompaniesIdStaffMembersTableColumnHelper.accessor(row => row.workPhone, {
      id: 'workPhone',
      header,
      cell: info => format(info.getValue()),
      size: width,
...
.with('mobilePhone', () => {
      ...
      return useCompaniesIdStaffMembersTableColumnHelper.accessor(row => row.mobilePhone, {
      id: 'mobilePhone',
      header,
      cell: info => format(info.getValue()),
      size: width,
...
.with('email', () => {
      ...
      return useCompaniesIdStaffMembersTableColumnHelper.accessor(row => row.email, {
      id: 'email',
      header,
      cell: info => format(info.getValue()),
      size: width,
...
.with('salutation', () => {
      const { label: header, format, width, minWidth } = modelConfig['salutation']

      return useCompaniesIdStaffMembersTableColumnHelper.accessor(row => row.salutation, {
      id: 'salutation',
      header,
      cell: info => format(info.getValue()),
      size: width,
...
export const useCompaniesIdStaffMembersTable = (args: UseCompaniesIdStaffMembersTableArgs) => {
  const rerender = useReducer(() => ({}), {})[1]
  ...
  const dataQuery = useGetApiCompaniesIdStaffMembers({
    ...args,
    pageNumber: pagination.pageIndex + 1,
    pageSize: pagination.pageSize
  })
