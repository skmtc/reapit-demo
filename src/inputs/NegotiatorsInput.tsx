...
import { useGetApiNegotiators } from '@/sections/Negotiators/services/Negotiators.generated.ts'
...
export const NegotiatorsInput = <Model extends FieldValues, Key extends KeyPath<Model>>({
  ...
  const negotiatorsResponse = useGetApiNegotiators({ name: queryText })
  ...
  return (
    <>
      ...
      <MultiSelectInput
        ...
        {...register(fieldName)}
        options={
          negotiatorsResponse?.data?._embedded
            ?.filter((option): option is Option => Boolean(option.id && option.name))
            ?.map(({ id, name }) => ({ value: id, name })) ?? []
        }
        defaultValues={defaultValue}
      />
      {errors.negotiatorIds?.message && <InputError message={errors.negotiatorIds.message as string} />}
    </>
  )
}
