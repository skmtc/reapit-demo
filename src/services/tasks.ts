...
export const postApiTasksFn = async ({ body }: UsePostApiTasksArgs) => {
  const res = await fetch(`/tasks/${querySerialiser({ args: {}, options: defaultQuerySerialiserOptions })}`, {
    method: 'POST',
...
export const patchApiTasksIdFn = async ({ 'If-Match': IfMatch, id, body }: UsePatchApiTasksIdArgs) => {
  const res = await fetch(`/tasks/${id}${querySerialiser({ args: {}, options: defaultQuerySerialiserOptions })}`, {
    method: 'PATCH',
