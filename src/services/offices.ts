...
export const postApiOfficesFn = async ({ body }: UsePostApiOfficesArgs) => {
  const res = await fetch(`/offices/${querySerialiser({ args: {}, options: defaultQuerySerialiserOptions })}`, {
    method: 'POST',
...
export const patchApiOfficesIdFn = async ({ 'If-Match': IfMatch, id, body }: UsePatchApiOfficesIdArgs) => {
  const res = await fetch(`/offices/${id}${querySerialiser({ args: {}, options: defaultQuerySerialiserOptions })}`, {
    method: 'PATCH',
