...
export const postApiAreasFn = async ({ body }: UsePostApiAreasArgs) => {
  const res = await fetch(`/areas/${querySerialiser({ args: {}, options: defaultQuerySerialiserOptions })}`, {
    method: 'POST',
...
export const patchApiAreasIdFn = async ({ 'If-Match': IfMatch, id, body }: UsePatchApiAreasIdArgs) => {
  const res = await fetch(`/areas/${id}${querySerialiser({ args: {}, options: defaultQuerySerialiserOptions })}`, {
    method: 'PATCH',
